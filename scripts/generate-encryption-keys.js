...
console.log("\n✅ Размеры:");
console.log(
  `   ENC_KEY_BASE64: ${key.length} байт (${key.length * 8} бит) -> ${keyBase64.length} символов в base64`
);
console.log(
  `   ENC_IV_BASE64:  ${iv.length} байт (${iv.length * 8} бит) -> ${ivBase64.length} символов в base64`
);
...
console.log(
  "   5. Для production используйте secrets management (AWS Secrets Manager, HashiCorp Vault)"
);
